...
def get_shifted_production(wine_data:pd.DataFrame):
    ...
    wine_data['prod_shift1'] = wine_data.groupby(['id_finca','variedad','modo'])['produccion'].shift()
    wine_data['prod_shift2'] = wine_data.groupby(['id_finca','variedad','modo'])['prod_shift1'].shift()
...
def get_production_changes(wine_data:pd.DataFrame):
    ...
    wine_data['prod_shift1_gt_shift2'] = [1 if x>y else 0 for x,y in zip(wine_data.prod_shift1, wine_data.prod_shift2)] 

    wine_data['prod_shift_max']=[x if x>y else y for x,y in zip(wine_data.prod_shift1,wine_data.prod_shift2)]

    wine_data['prod_shift_change'] = [x-y for x,y in zip(wine_data.prod_shift1, wine_data.prod_shift2)] 

    wine_data['prod_shift_avg']=[(x+y)/2 if y!=-1 else x for x,y in zip(wine_data.prod_shift1,wine_data.prod_shift2)]

    return wine_data


def get_production_change_by_estacion(wine_data:pd.DataFrame):
    prod_change_by_estacion = pd.DataFrame(wine_data.groupby(['campaña','id_estacion']).agg(prod_est_mean_change=('prod_shift1_gt_shift2', 'mean'))).reset_index()

    wine_data = wine_data.merge(prod_change_by_estacion, how='left', on=['campaña','id_estacion'])
...
def get_shifted_production_he_changes(wine_data:pd.DataFrame):
    ...
    wine_data['prod_he_shift_change'] = [x-y for x,y in zip(wine_data.prod_he_shift1, wine_data.prod_he_shift2)] 
    wine_data['prod_he_shift_avg']=[(x+y)/2 if y!=-1 else x for x,y in zip(wine_data.prod_he_shift1,wine_data.prod_he_shift2)]
...
def get_historic_prod_he_by_var_and_zone(wine_data):
    dfs = []
    from_period, to_period = wine_data['campaña'].min()+1, wine_data['campaña'].max()+1
...
def get_shifted_production_by_var(wine_data):
    prod_by_finca_var = wine_data.groupby(['campaña','id_finca','variedad'])['produccion'].sum().reset_index()
...
def get_shifted_production_by_finca(wine_data):
    ...
    prod_by_finca = wine_data.groupby(['campaña','id_finca'])['produccion'].sum().reset_index()
    ...
    return wine_data.merge(
        prod_by_finca[['campaña','id_finca','prod_finca_shift_1','prod_finca_shift_2']],
...
def get_shifted_production_he_by_var(wine_data):
    ...
    return wine_data.merge(
        prod_he_by_var[['campaña','variedad', 'modo', 'prod_he_var_mean','prod_he_var_std']],
...
def get_shifted_production_he_by_zone(wine_data):
    ...
    return wine_data.merge(
        prod_he_by_zone[['campaña','variedad', 'modo','id_zona','prod_he_zone_mean','prod_he_zone_std']],
